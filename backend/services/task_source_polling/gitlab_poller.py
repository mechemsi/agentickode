...
"""Polls GitLab issues for a project and creates TaskRuns for open ai-task issues."""
...
logger = logging.getLogger("agentickode.polling.gitlab")

_AI_TASK_LABEL = "ai-task"
_USE_CLAUDE_LABEL = "use-claude"


class GitLabIssuePoller:
    """Pulls open GitLab issues labeled ``ai-task`` and dispatches TaskRuns."""
    ...
    async def poll(self, project: ProjectConfig, session: AsyncSession) -> list[int]:
        ...
        provider = get_git_provider("gitlab", get_http_client(), access_token=token)
        repo_path = f"{project.repo_owner}/{project.repo_name}"

        try:
            issues = await provider.list_issues(repo_path, state="open", limit=100)
        except Exception as exc:
            logger.warning("GitLab poll failed for %s: %s", repo_path, exc)
            return []
        ...
        # GitLab issue identifier is iid (project-local); GitLabProvider.list_issues maps it to "number"
        task_ids = [str(i["number"]) for i in candidates]
        already = await existing_task_ids(session, project.project_id, "gitlab", task_ids)
        ...
        for issue in candidates:
            ...
            logger.info("GitLab poll: created run #%d for %s!%s", run.id, repo_path, task_id)
        return created

...
class ProjectConfig(Base):
    ...
    monitoring_rules = relationship(
        "MonitoringRule", back_populates="project", cascade="all, delete-orphan"
    )
...
class ProjectWorkspaceServer(Base):
    ...
    project_id = Column(
        Text, ForeignKey("project_configs.project_id", ondelete="CASCADE"), primary_key=True
    )
    workspace_server_id = Column(
        Integer, ForeignKey("workspace_servers.id", ondelete="CASCADE"), primary_key=True
    )

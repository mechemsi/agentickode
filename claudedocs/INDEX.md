...
| [Replace workflow templates with flow prompts](plans/2026-06-08-workflows-to-flow-prompts.md) | implemented | 2026-06-08 | Dropped `WorkflowTemplate` + `PhaseExecution` engine; every run is a single flow-prompt agent call (ADR-009, all 5 phases shipped). |
...
| [Flow prompts — Phase 5 (remove engine)](implementations/2026-06-10-flow-prompts-phase5.md) | 2026-06-10 | ADR-009 Phase 5: removed the `WorkflowTemplate`/`PhaseExecution` engine + flag; migration 044 drops the tables (irreversible); flow prompts unconditional |
